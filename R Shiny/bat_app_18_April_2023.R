...
dat <- readRDS("final_df_for_tool_exposure_BATS_2011_2020_18Nov2022.RDS")
rownames(dat) <- NULL
...
dat <- rename(dat, LL95CrI = LL95CI) # These are actually credible intervals, not confidence intervals
dat <- rename(dat, UL95CrI = UL95CI)

tib1 <- dat

######################
# Probability of death
dat <- readRDS("final_df_for_tool_death_BATS_2011_2020_18Nov2022.RDS")
rownames(dat) <- NULL
...
# Sort the species alphabetically so they look nice in the tool
#us_states <- as.character(unique(dat$State))
#us_states <- sort(us_states)

# Create a dataframe to compare estimated risk to threshold
plot_threshold <- data.frame(med_est = 0.0004,
                             lower_ci = NA,
                             upper_ci = NA,
                             type = "Threshold")
...
shinyApp(
  ui = fluidPage(
    ...
    titlePanel(
      h1("Quantitative risk assessment for rabies\n(bat species version)", align = "center")
    ),
    
    div(img(src='shutterstock_1713263746-0_crop_small.png'), style="text-align: center;"), # Downloaded from Shutterstock. Contributor: Studio Ayutaka. https://www.shutterstock.com/image-vector/various-bats-hanging-upside-down-row-1713263746. License: Limited usage in print, advertising, and packaging. Unlimited web distribution.
    ...
    p("This app is specific to bat species. Although the average person will likely not be able to 
      identify the species of bat to which they were exposed, in some locations, such as in the nation's
      national parks, park rangers may have expert knowledge about the species of bats found in 
      particular areas."),
    
    h2("Purpose"),
    
    p("The aim of the tool is to promote judicious use of post-exposure prophylaxis (PEP).
      It estimates (i) the probability that a bat would test positive for rabies virus (RABV) given that a person 
      was exposed, Pr(rabid|exposure) and (ii) the probability that a person would die from rabies given that they were exposed, Pr(death|exposure)."),
    ...
    tabsetPanel(
      tabPanel("Pr(rabid|exposure)", 
               ...
               selectizeGroupUI(
                 ...
                 params = list(
                   ...
                   CommonName = list(inputId =  "CommonName", 
                                     title = "Which bat is responsible for the suspected rabies exposure (common name)?", 
                                     placeholder = 'select'),
                   Species = list(inputId = "Species", 
                                  title = "Which species of bat is responsible for the suspected rabies exposure?", 
                                  placeholder = 'select'),
                   Provoked = list(inputId = "Provoked", 
                                   title = "What circumstances led to the rabies exposure?", 
                                   placeholder = 'select'),
                   Healthy = list(inputId = "Healthy", 
                                  title = "What is the health status of the bat?", 
                                  placeholder = 'select')
                 )
               ),
               ...
               h2("Interpretation: Pr(rabid|exposure)"),
               
               p("The estimated risk threshold is", strong("0.0004."),
                 "If Pr(rabid|exposure) < risk threshold, PEP may not be needed; however, if Pr(rabid|exposure) > risk threshold OR 
      Pr(rabid|exposure) overaps with the risk threshold, PEP may be indicated. Consider consulting your state health department
      or CDC rabies experts."),
      
      p("Enter the values from the table above to compare the risk to the study-derived threshold:"),
      
      bootstrapPage(div(style="display:inline-block", numericInput(inputId="lowerci", 
                                                                   label="Lower 95% CrI", value = 0, min = 0, max = 1, step = 0.0001)),
                    div(style="display:inline-block", numericInput(inputId="med", 
                                                                   label="Median", value = 0, min = 0, max = 1, step = 0.0001)),
                    div(style="display:inline-block", numericInput(inputId="upperci", 
                                                                   label="Upper 95% CrI", value = 0, min = 0, max = 1, step = 0.0001))),
      plotOutput("plot_comparison", width = 800)
      ...
      tabPanel("Pr(death|exposure)", 
               ...
               h2("Interpretation: Pr(death|exposure)"),
    ...
    p("Parameter estimates for the location and severity of rabies exposures came from the literature.
      The positivity rates for bats as well as estimates regarding the circumstances
      of the exposure (provoked and health status) came from the national rabies surveillance 
      system in the US (10 years of data from 2011-2020)."),
...
p("Image downloaded from Shutterstock under a Standard License 
  ...
  a(href="https://www.shutterstock.com/image-vector/various-bats-hanging-upside-down-row-1713263746.",
    "https://www.shutterstock.com")),

h2("Contributors"),

p("Kelly Charniga: initial version"),

h2("Contact information"),

p("Ryan Wallace, euk5@cdc.gov")
...
server = function(input, output, session) {
  ...
  # Take the values the user puts in the text boxes and add them to dataframe
  # containing risk threshold
  to_plot2 <- reactive({
    
    datt <- rbind(plot_threshold, 
                  data.frame(med_est = input$med,
                             lower_ci = input$lowerci,
                             upper_ci = input$upperci,
                             type = "Estimated risk"))
  ...
  # Display the risk comparison plot
  output$plot_comparison <- renderPlot({
    
    ggplot(to_plot2(), aes(x = med_est, y = type)) + 
      geom_errorbarh(aes(xmin = lower_ci, xmax = upper_ci), height = 0.25, linewidth = 1) + 
      geom_point(size = 2) +
      # The next line makes everything break
      #scale_color_manual(values = c("#b8396b", "#76bae0"))+ 
      theme_bw() +ylab("") +xlab("Pr(rabid|exposure)") +
      theme(legend.position="none") +theme(axis.text=element_text(size=18),
